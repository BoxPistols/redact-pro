...
const parserNames = [
  'parsePDF', 'parseDOCX', 'parseXLSX', 'parseCSV', 'parseTXT',
  'parseMD', 'parseHTML', 'parseRTF', 'parseJSON', 'parseODT'
];
...
function cr(h1, h2) { const l1=luminance(h1),l2=luminance(h2); return (Math.max(l1,l2)+0.05)/(Math.min(l1,l2)+0.05); }
...
if (darkMatch) {
  ...
  for (const [label, ratio, min, max] of checks) {
    ...
    console.log(`${ok?'✅':'❌'} ${label}: ${ratio.toFixed(2)} (${min}-${max===99?'∞':max})`);
    if (!ok) fail++;
    else pass++;
  }
}
